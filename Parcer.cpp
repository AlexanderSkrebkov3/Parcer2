			...
			while (1) // Перебираем символы разделители пропускаем
			{
				...
				if (s[k1] != ' ' && s[k1] != '\n' && s[k1] != '\r' && s[k1] != '\t') // Ищем не разделитель
				{
					...
					//	any_cast<unordered_map<string, any> *>(data)->insert({ key, any_cast<any>(Json(sOb)) });
						if (flag) Ob1.insert({ key, Json(sOb) }); // Рекурсивный вызов конструктора для вложенного объекта
						else Ob2.push_back(Json(sOb));// Рекурсивный вызов конструктора для вложенного объекта
...
int main()
{
	...
	try
	{
		...
		//	Json Js1(teststr); // Чтение из строки
		Json Js1 = Json::parseFile("Proba.txt"); // Чтение из файла
		Js1.print();
		// Обращение в элементам Объекта
		cout << endl << "Обращение к карте по ключу lastname : " << any_cast<string&>(Js1["lastname"]);
		cout << endl << "Обращение к карте по ключу islegal: " << any_cast<bool>(Js1["islegal"]);
		cout << endl << "Обращение к карте по ключу age: " << std::any_cast<int>(Js1["age"]);

		auto marks = std::any_cast<Json>(Js1["marks"]); // Получаем массив оценок
		// Обращение в элементам массива
		for (int i = 0; i < 6; i++)
		{
			cout << endl << "Обращение к массиву по индексу marks: " << std::any_cast<int>(marks[i]);
		}
		
		//cout << endl << "Обращение к массиву по индексу: " << std::any_cast<int>(marks[1]);


		auto address = std::any_cast<Json>(Js1["address"]); // Получаем вложенный объект адрес
		cout << endl << "Обращение к карте по ключу city: " << any_cast<string>(address["city"]);
		cout << endl << "Обращение к карте по ключу street: " << std::any_cast<string>(address["street"]);
